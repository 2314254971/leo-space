...
```json
{
  ...
  ], // 模型架构配置，模型结构一般预先定义好了，并合并到了库内，后续可直接通过该字段配置模型结构，若不存在，则需要自定义模型结果（继承基类），将py文件共同放置模型目录下
  ...
  "intermediate_size": 11008,
  "max_position_embeddings": 128000,//最大上下文
...
这是QWEN2.5VL-3B（[千问2.5-VL-3B-Instruct · 模型库](https://www.modelscope.cn/models/Qwen/Qwen2.5-VL-3B-Instruct/files)）模型的config文件，
...
以`mistralai/Mistral-7B-Instruct-v0.1`模型的输出为例：
...
![img](https://pic3.zhimg.com/v2-2a9b8feb5ad4c372c34aa181f4fd1e8e_1440w.jpg)
...
参考：[【搭建框架必备基础】彻底搞懂Chat template - 知乎](https://zhuanlan.zhihu.com/p/24450011474)

聊天模板主要由jinja模板字符串组成，以llama聊天模板为例：
...
我们可以看出，上述jinja2模板实际上对输入字符串做了一个“后结构化解析”，将角色等信息替换成词条中专用token，让分词器可以正常分词，不至于将额外信息当作输入词汇进行拆解（大模型本质上是输入字符预测下一个字符，因此需要将输入数据组装成完成的、和训练数据一致的字符串）
...
- **位置编码的容量限制**：
  ...
  - **相对位置编码**（如RoPE、ALiBi）：虽支持**外推**，但训练时使用的最大长度（如2048）决定了模型对长距离关系的建模能力。超出训练长度时，位置关系的计算可能失效（例如RoPE外推时出现注意力分数混乱）。（简单来讲，若输入的文本长度超出训练的文本长度，则位置编码数值分布超出训练集规律，此时模型预测结果不可控）
- **注意力机制的计算复杂度**：
...
若可依赖外部能力：一般做rag，对文本块拆分

若不依赖外部能力：纯粹靠模型，则需要做长文本微调（效果预期最好），又或者做位置编码外推（只能缓解问题，输出效果可能会不理想）