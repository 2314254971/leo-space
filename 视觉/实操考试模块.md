| **模块**           | **考点**                                                     |
| ------------------ | ------------------------------------------------------------ |
| 模块一（数据标注） | 数据清洗与去重优化  工程专业名词标注正确性  工程大场景多尺度目标标注边框精准度  工程场景遮挡情况下，目标标注方法  多尺度目标标注覆盖完整性  版本迭代与数据追溯  密集场景ID追踪与漏标率  运动模糊/光照干扰标注容错  半自动标注工具效能评估 |
| 模块二（模型训练） | 数据增强策略与域适配性  锚框聚类与匹配优化  损失函数权重优化平衡  多尺度特征融合效率  基于工程数据集的预训练模型优化调参技巧  工程场景类别样本不均衡及重采样  超参数动态调度（学习率/动量）  过拟合抑制（正则化/早停）  模型结构适配性与优化  推理速度-精度协同调优 |
| 模块三（模型部署） | 硬件加速库适配，提高工程图片检测效率  量化压缩与精度损失平衡  多线程/异步推理资源竞争  运行时日志与异常监控体系  模型热更新与版本回滚  模型api整理  模型轻量化（蒸馏/剪枝等）  网页端接口联调  模型队列控制及资源优化 |
