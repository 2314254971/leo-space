...
### labelimg（个人感觉比labelme好用）

默认支持voc和yolo标签导出，而labelme需要pro版本才支持
...
def nms(boxes, scores, iou_threshold=0.5):
    """
    NMS 非极大值抑制
    :param boxes: ndarray, shape=(N, 4), 格式 [x1, y1, x2, y2]
...
图片来源：https://mukdrpaizj.feishu.cn/wiki/VV0wwXzPOirxm6kQHoSc9DmanJf?from=from_copylink

![](.\asset\yolo-world.png)

简单来说yolo-world就是在最后检测层做文章，假设模型预设有C个类别，那么分类预测的这一步骤分卷积核参数就是【Cin,C,1,1】，输入特征图经过卷积后通道数由Cin变为C，即预测C个类别的分数。

此处，换一个视角看待最后分类卷积的操作，将其等价地转换成多模态相似度对齐的操作，即把类别信息看作是一个token，该token的嵌入深度为Cin，词表大小为C，即把分类卷积变成了embedding矩阵，最后的 类别预测/分类 变成了**类别信息（类似于文本）对齐图像特征**，即多模态问题

这样一来，理论上就可以处理无穷多的类别

如何拓展到无穷类别：初始化一个大的类别embedding矩阵，训练时按需导入即可不断拓展，无需一次性导入所有类别计算
